...
import { X, ExternalLink, Sparkles, Calendar, Layers } from "lucide-react";
import { CoursicleScheduleView } from "./coursicle-schedule-view";
...
export function CoursicleModal({ isOpen, onClose, onViewOnPage }: Props) {
  useEffect(() => {
            ...
            <div>
              <div className="flex items-center space-x-2">
                <h3 className="text-base font-black text-slate-900 dark:text-white">
                  Coursicle &amp; Google Calendar Example
                </h3>
                <span className="rounded-full bg-emerald-100 dark:bg-emerald-950/80 px-2 py-0.5 text-[10px] font-extrabold text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                  Live View
                </span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">
                Full example of classes, color-coding, desktop &amp; mobile Google Calendar synchronization.
              </p>
            </div>
          </div>
